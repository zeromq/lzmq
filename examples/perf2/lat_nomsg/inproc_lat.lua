...
local local_lat = zthreads.run(ctx, "@./local_lat.lua", addr, message_size, message_count, argv[3])
local_lat:start()

ztimer.sleep(1000)

local remote_lat = zthreads.run(ctx, "@./remote_lat.lua", addr, message_size, message_count, argv[3])
remote_lat:start()
